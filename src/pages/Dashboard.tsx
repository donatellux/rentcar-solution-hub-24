  ...
  const fetchDashboardStats = async () => {
    ...
    try {
      ...
      const { data: reservations } = await supabase
        .from('reservations')
        .select('prix_par_jour, date_debut, date_fin')
        .eq('agency_id', user.id)
        .eq('statut', 'active');

      // Fetch maintenance alerts (vehicles that need maintenance)
      const { data: maintenanceData } = await supabase
        .from('vehicles')
        .select('kilometrage, km_last_vidange, vidange_periodicite_km')
        .eq('agency_id', user.id);
      ...
      const maintenanceAlerts = maintenanceData?.filter(vehicle => {
        if (vehicle.kilometrage && vehicle.km_last_vidange && vehicle.vidange_periodicite_km) {
          ...
          return kmSinceLastMaintenance >= vehicle.vidange_periodicite_km * 0.9; // Alert at 90%
        }
        return false;
      }).length || 0;
